...
| 2026-09-06 | [0039](0039-shared-persistence-adapters-share-ports-not-a-jdbc-core.md) | Accepted contract | Extends ADR 0014 to deployments whose processes are not on one host: a durable adapter is a standalone module sharing the ports, the closed capability set, the sealed failure vocabularies and the conformance suite, and sharing no production code; correctness rests on database transactions and row-level compare-and-set; all durable stores of one deployment address one database; migration is serialized by a database advisory lock. |
