...
- A recorded terminal result is retained in the configured store, keyed by tenant and traversal, and its payload reports one of four distinct states rather than a nullable field: produced-and-kept, produced-and-refused-for-size, produced-and-unrepresentable, or never produced. A read carries a fourth answer, `Redacted`, for a payload that was never retainable, distinct from `Expired` for one that aged out after being retained.
