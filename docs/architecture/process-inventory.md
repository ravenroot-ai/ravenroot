...
- Pagination orders by `(createdAt descending, processInstanceId descending)`. Both components are immutable per row once set, so a row cannot move between pages of an in-flight scan.
