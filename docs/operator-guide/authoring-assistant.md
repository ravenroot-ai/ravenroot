...
| `RAVENROOT_ASSISTANT_MAX_OUTPUT_TOKENS` | Whole number from 1 to 16,000; default 16,000 | Output ceiling for each provider turn. On providers that count reasoning as output, the ceiling covers reasoning and visible text together. An explicit value may tighten the deployed default but cannot raise it. |
| `RAVENROOT_ASSISTANT_MAX_TOOL_ITERATIONS` | Whole number from 1 to 8; default 8 | Maximum provider turns used by one author message, including turns that request read-only tools. An explicit value may tighten the deployed default but cannot raise it. |
...
These two limits are live assistant-session policy read at startup. They are not the `maxTokens`
properties of `llm-prompt` or `agent` graph nodes, and they are not stored, placed in an execution
manifest, or pinned to a durable execution. Restart or recreate the server to apply a change. The
environment ranges are operator ceilings that preserve the previous deployed maxima; public Java
construction remains a separate compatibility surface.
