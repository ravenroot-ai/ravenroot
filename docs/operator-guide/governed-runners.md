...
| `ravenroot.runner.control` | Cancellation and liveness reconciliation; user principals only |
...
actor and revision. Recovery exposes bounded observation/conflict/unknown counters through its
embedded metrics snapshot; monitor the durable journal and failed worker protocol counters as well.
...
and use operator-led recovery; a terminal runner report is not evidence that every graph branch ran.
...
control-plane cleanup. Minimal lock metadata may remain to preserve cross-process synchronization.
