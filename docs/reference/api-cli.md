...
| `GET /v1/executions/{id}` | State or terminal result, read from a process-local cache first and a durable, tenant-and-traversal-keyed result store on a miss, so a result readable before a restart or from another instance is readable after one wherever a durable, result-capable store is composed. `paused` distinguishes a held execution from an ordinary running one, always `false` once terminal. `terminationReason` and `cancelled` qualify a terminal `status` the same way `paused` qualifies `RUNNING` — always present (`null`/`false` when nothing distinguishes the termination), and carried on the `200` body and on both `410` bodies below. A cancelled execution reports `status=FAILED`; read `terminationReason` beside it, never `status` alone. `410 EXECUTION_RESULT_EXPIRED` reports an execution whose result aged past its retention deadline; `410 EXECUTION_RESULT_REDACTED` reports one whose payload was never retained in the first place — refused for exceeding the store's byte cap, or for not projecting onto the closed payload model at all — and additionally carries `payloadState` (`WITHHELD` or `UNCONVERTIBLE`) naming which. Both `410` bodies still carry `status`, `terminationReason` and `cancelled` |
