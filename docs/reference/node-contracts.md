...
`PublishedNodeContractTest` from compiled Java descriptors; dynamic group fields use `<N>` for a
canonical positive, contiguous item index. The same test parses and schema-checks
...
rows publish its unbounded atomic `skills.<N>.*` additional-property group.
...
| `mcpServers` | MCP servers | Comma-separated names of MCP servers this deployment declared in its environment (RAVENROOT_MCP_SERVER_<hex(name)>). Their tools are offered to the model as <server>__<tool>. The executing runtime applies its configured admission ceiling. | `STRING` | false | Not declared | Not declared | false | Not declared | Not declared | Not declared |
| `maxTurns` | Max turns | Model turns this run may take before it is refused. Defaults to the executing runtime default and may not exceed its configured ceiling. | `INTEGER` | false | Not declared | Not declared | false | Not declared | Not declared | Not declared |
...
| `reviewPresentationVersion` | Review content | Version one deliberately discloses selected inert plain text only on authorized exact-task detail. | `STRING` | false | Not declared | 1 | false | confirmationPresentationVersion:EQUALS:1 | Not declared | Not declared |
| `reviewTextSource` | Review text source | Payload or a dotted payload path selecting the exact text to review; no interpolation or coercion. | `STRING` | false | payload | Not declared | false | reviewPresentationVersion:EQUALS:1 | Not declared | max UTF-8 bytes 1024 |
| `reviewMaxUtf8Bytes` | Maximum review text bytes | Inclusive UTF-8 byte ceiling; oversized content refuses task creation without truncation. | `INTEGER` | false | 65536 | Not declared | false | reviewPresentationVersion:EQUALS:1 | Not declared | min 1; max 262144 |
