...
| Cancel is refused | First check whether responder enforcement is enabled. When disabled, any admitted same-tenant principal may cancel; when enabled, compare the qualified identity with the requester. | Correct the configured mode or re-authenticate as the requester. Use explicit override authority only for an exceptional admin action. |
...
| Form returns `INVALID_REQUEST` | Compare every field name/type, required value, enum member, date/time format, numeric minimum/maximum, multiline/text byte limit, and response byte limit with the pinned schema. | Submit the exact closed typed map; omit unknown fields and executable/markup content. |
| Registered presentation is unavailable | Check `registeredPresentationsEnabled`, registry readability, profile id/version/kind, safe launch origin, and that every custom or external origin differs from Workbench. | Repair the operator registry and restart; the task remains open. |
