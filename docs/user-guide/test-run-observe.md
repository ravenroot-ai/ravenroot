...
1. Use **Test** first: it is the UI default and bypasses behavior effects while retaining traversal evidence. Test executes an immutable GraphML snapshot and keeps monitoring in the originating editor document, so its identity, edits, selection, and viewport stay in place.
   Before Test or Run, the Workbench asks the runtime to inspect that same serialized GraphML. Local
   validation is advisory; the runtime is authoritative and revalidates the bytes when the start is
   submitted. A refusal creates one activity row, selects the affected node/property when available,
   and shows the incident handle when the cause is intentionally generic.
