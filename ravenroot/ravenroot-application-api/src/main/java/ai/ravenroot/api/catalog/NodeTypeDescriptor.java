 ...
 *
 * @param defaultNature        the runtime nature a node of this type has when
 *                             it declares none, or
 *                             {@code null} for a descriptor that says nothing —
 *                             see
 *                             {@link #effectiveDefaultNature()} for why the
 *                             null is kept rather than
 *                             normalised to {@link NodeRuntimeNature#WORKER}
 * @param allowedNatures       the natures graph content may choose from, or
 *                             empty for a descriptor that
 *                             says nothing — see
 *                             {@link #effectiveAllowedNatures()} for the
 *                             fail-closed
 *                             reading of empty
 * @param commands             the application commands this behavior
 *                             <em>admits</em> — an allowlist, not a
 *                             suggestion list; see the note on
 *                             {@link #commands()} below
 * @param outcomes             the outcomes this behavior can produce, empty for
 *                             a descriptor that
 *                             says nothing — see
 *                             {@link #resolveOutcomes(java.util.function.Function)}
 * @param behavior             unique behavior identifier used to select the
 *                             node implementation
 * @param displayName          editor-facing name; defaults to {@code behavior}
 *                             when absent
 * @param category             editor grouping; defaults to {@code General} when
 *                             absent
 * @param description          human-readable behavior description, or the empty
 *                             string when absent
 * @param visualType           renderer hint; defaults to {@code actor} when
 *                             absent
 * @param agentic              whether the behavior is eligible for
 *                             agent-oriented presentation
 * @param properties           immutable property descriptors accepted by this
 *                             node type
 * @param capabilities         immutable capabilities declared by the behavior
 * @param runtimeConcurrency   trusted default and ceiling for platform-owned
 *                             per-node admission
 * @param additionalProperties immutable dynamic property collections accepted
 *                             by this node type
 */
public record NodeTypeDescriptor(
    ...
    /**
     * Normalizes optional display metadata and rejects outcome/nature combinations
     * that a graph could
    ...
    public NodeTypeDescriptor {
        ...
        // Absent stays absent. Normalising a null default to WORKER here would erase
        // the difference
        // between "this descriptor said nothing" and "this descriptor declared WORKER",
        // and
        // BehaviorRegistry's source-capability derivation depends on exactly that
        // difference: it may
        // supply SOURCE to a descriptor that said nothing, and must refuse a descriptor
        // that said
        // WORKER while implementing InboundSourceCapable. Collapse the two and every
        // legacy source
        // package looks like a contradiction. The same reasoning applies to property
        // conditions.
        allowedNatures = allowedNatures == null ? Set.of() : Set.copyOf(allowedNatures);
        commands = commands == null ? Set.of()
                : commands.stream()
        ...
        // A property-derived outcome that names a property this descriptor does not
        // declare cannot be
        // resolved by anyone: the editor has no field to read and no default to fall
        // back to, so the
        // outcome would silently vanish from the suggestions instead of being wrong
        // loudly. Catching it
        // at construction makes it a defect in the behavior that declared it, found the
        // first time its
    ...
    /**
     * The nature a node of this type has when it declares none: the declared
     * default, or
     * {@link NodeRuntimeNature#DEFAULT}.
     *
     * <p>
     * This is the accessor every consumer should use. {@link #defaultNature()}
     * answers the
     * different question of whether the descriptor said anything, which only
    ...
    public Set<NodeRuntimeNature> effectiveAllowedNatures() {
        return allowedNatures.isEmpty()
                ? effectiveDefaultNature() == NodeRuntimeNature.WORKER
                        ? Set.of(NodeRuntimeNature.WORKER, NodeRuntimeNature.TRAVERSAL)
                        : Set.of(effectiveDefaultNature())
                : allowedNatures;
    }
     ...
     *
     * <h4>This is an admission allowlist, and it is empty for every built-in on
     * purpose</h4>
     * <p>
     * It reads like a list of commands a behavior "supports", and the editor does
     * use it to suggest
     * values for an edge's Command field. That is a secondary use. Its primary use
     * is enforcement:
     * {@code GraphRunner} refuses a delivery whose command carries {@link
     * ai.ravenroot.api.execution.NodeDirective#APPLICATION} unless the target's
     * descriptor lists that
     * command's name, both at admission time over the reachable graph and again on
     * the delivery itself.
     * {@link ai.ravenroot.api.execution.NodeCommand} states the same contract:
     * named commands are inert
     * until the target's trusted catalog descriptor admits them.
     * </p>
     *
     * <p>
     * So an empty datalist in the editor is not an oversight to be filled in. None
     * of
     * the nine built-in behaviors interprets an application command — each one does
     * its single job and
     * ignores the structural instruction — and adding names here to populate a
     * suggestion list would
     * widen a security allowlist to admit deliveries no built-in handles, in
     * exchange for a UI
     * convenience. The suggester is empty because there is genuinely nothing to
     * suggest; it fills in
     ...
     * @return copy retaining all catalog metadata with the supplied nature
     *         constraints
     */
     ...
     *
     * <p>
     * A builder rather than a twelfth argument at every call site: a behavior's
     * outcomes belong
     * next to nothing else in the descriptor, and the alternative was to make nine
     * built-in factories
     * spell out {@code null, Set.of(), Set.of()} for the three components between
     * {@code capabilities}
     * and {@code outcomes} that they do not declare. Validation is not skipped —
     * this constructs
     * through the canonical constructor, so a property-derived outcome naming a
     * property this
    ...
    /**
     * Every outcome a node of this type can produce, resolved against that node's
     * configured
     * properties.
     *
     * <p>
     * Parameterization is why this takes a lookup rather than being a constant:
     * {@code cel-decision}
     * and {@code http-request} read their outcome names out of node properties, so
     * the answer is a
     * property of the <em>node</em>, not of the type. {@code propertyValues}
     * returns the raw value the
     * node carries under a property name, or {@code null} when it carries none —
     * the same distinction
     * {@code NodeProperties.string} makes, and
     * {@link NodeOutcomeDescriptor#resolve} depends on it.
     * </p>
     *
     * <p>
     * The result is deliberately a set: two properties set to the same name are one
     * outcome, because
     * one outcome is all {@code nextEdges} can distinguish.
     * </p>
     *
     * <p>
     * <strong>This is not the set of outcomes a traversal can act on.</strong> A
     * runner that finds no
     * edge matching the produced outcome retries the lookup with {@code continue},
     * so an edge wired to
     * {@code continue} may fire for an outcome that is not in this set at all. Any
     * reader deciding that
     * an edge is unreachable must exempt {@code continue} for that reason.
     * </p>
     *
     * <p>
     * <strong>An empty set means "this descriptor says nothing", never "this
     * behavior emits
     * nothing".</strong> That reading is the one with the most ways to go wrong:
     * most node packages
     * declare no outcomes — seven of the nine extension modules shipped here, and
     * any package written
     * against an older version of this record — so a consumer treating empty as a
     * closed set would
     * call every edge leaving every one of them unreachable. Empty is the absence
     * of an answer.
     * </p>
     *
     * <p>
     * <strong>For a behavior in this repository that does declare outcomes, the
     * resolved set is
     * exact</strong>, and the editor-side check relies on that. The qualification
     * this paragraph
     * used to carry — that an outcome property may be "bound to a value only known
     * at run time" — is
     * not true of any behavior shipped here: {@code NodeProperties.render} is
     * applied to {@code url},
     * {@code body}, {@code source}, {@code template}, {@code prompt} and
     * {@code objective}, and to no
     * outcome property at all. {@code cel-decision}, {@code http-request} and the
     * SpEL decision node
     * read
     * {@code trueOutcome}/{@code falseOutcome}/{@code successOutcome}/{@code failureOutcome}
     * verbatim at composition time.
     * </p>
     *
     * <p>
     * It remains true of a <em>third-party</em> behavior, which is free to pass its
     * own outcome
     * property through a template renderer before emitting it. For that one the
     * value resolved here is
     * the template text and not the outcome produced, so a consumer turning this
     * set into a verdict
     * owes that case an exemption too — see {@code node-outcomes.js}, which treats
     * a resolved name
    ...
    /**
     * The canonical shape before {@code defaultNature} and {@code allowedNatures},
     * retained
     * so that node packages and plugin bundles already compiled against it keep
     * linking.
     *
     * <p>
     * Additive, not breaking — but only because this overload exists. Adding a
     * record component
     * changes the canonical constructor's descriptor, which is a <em>binary</em>
     * break for any caller
     * already compiled against the older class file and not merely a source break
     * for one being
     * recompiled. That claim is proved rather than asserted here: see
     * {@code NodeTypeDescriptorBinaryCompatibilityTest}, which also carries a
     * negative control so
     * "every published shape still links" cannot be confused with "the harness
     * detects nothing".
     * </p>
     *
     * <p>
     * Both nature components default to absent, which is the correct reading of a
     * descriptor
     * written before the concept existed: it said nothing, so its nodes are workers
     * unless the
     ...
     * @param category     editor grouping, normalized to {@code General} when
     *                     absent
     * @param description  author-facing explanation, normalized to the empty string
     *                     when absent
     * @param visualType   renderer hint, normalized to {@code actor} when absent
     * @param agentic      whether agent-oriented presentation is allowed for the
     *                     behavior
     ...
     * @param category       editor grouping, normalized to {@code General} when
     *                       absent
     * @param description    author-facing explanation, normalized to the empty
     *                       string when absent
     * @param visualType     renderer hint, normalized to {@code actor} when absent
     * @param agentic        whether agent-oriented presentation is allowed for the
     *                       behavior
     * @param properties     immutable behavior-property schema
     * @param capabilities   immutable capabilities advertised by the behavior
     * @param defaultNature  nature used when the graph omits
     *                       {@code runtime.nature}, or {@code null}
     *                       when this versioned shape says nothing about nature
     * @param allowedNatures natures graph content may select, or empty when nature
     *                       is undeclared
     */
    public NodeTypeDescriptor(String behavior, String displayName, String category, String description,
            String visualType, boolean agentic, List<NodePropertyDescriptor> properties,
    ...
    /**
     * Compatibility constructor preserving the eleven-argument canonical
     * descriptor, which was the
     * canonical shape before {@code outcomes}.
     *
     * <p>
     * Same reasoning as the two overloads above, and the same proof: adding a
     * record component
     * changes the canonical constructor's descriptor, which is a binary break for
     * any node package or
     * plugin bundle already compiled against the eleven-argument shape.
     * {@code NodeTypeDescriptorBinaryCompatibilityTest} carries this shape as a
     * published case.
     * </p>
     *
     * <p>
     * Declaring no outcomes is the correct reading of a descriptor written before
     * the concept
     ...
     * @param allowedNatures natures graph content may select, or empty when nature
     *                       is undeclared
     * @param commands       application commands admitted by the behavior; empty
     *                       for none
     */
    public NodeTypeDescriptor(String behavior, String displayName, String category, String description,
            String visualType, boolean agentic, List<NodePropertyDescriptor> properties,
     ...
     * @param agentic        whether the editor may present the behavior as
     *                       agent-oriented
     * @param properties     immutable property descriptors accepted by the node
     *                       type
     * @param capabilities   immutable capabilities declared by the behavior
     * @param defaultNature  runtime nature selected when graph content declares
     *                       none
    ...
    /**
     * Compatibility constructor preserving the thirteen-argument canonical
     * descriptor.
     *
     * @param behavior           unique identifier used to select the node
     *                           implementation
    ...
    /**
     * Sorted identifiers of {@link #effectiveAllowedNatures()}, for stable
     * serialization.
     *
     * @return immutable identifiers in {@link NodeRuntimeNature} declaration order,
     *         including the
