...
public final class RunnerRecoveryLoop implements AutoCloseable {
    ...
    public RunnerRecoveryLoop(RunnerJobService jobs, PinnedRunnerContinuationExecutor continuations,
        ...
        if (tenants.isEmpty() || tenants.size() > 256) throw new IllegalArgumentException("bounded runner recovery tenants required");
    }
    public void start() { scheduler.scheduleWithFixedDelay(this::sweep, 1, 2, TimeUnit.SECONDS); }
