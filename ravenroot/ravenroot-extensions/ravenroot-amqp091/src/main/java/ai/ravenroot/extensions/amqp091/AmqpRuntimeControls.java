...
final class AmqpRuntimeControls {
    static final AmqpRuntimeControls PRODUCTION = new AmqpRuntimeControls(System::nanoTime,
            task -> Thread.ofVirtual().name("ravenroot-amqp091-", 0).start(task), 32, 16, 4_096);
    ...
    AmqpRuntimeControls(LongSupplier ticker, Executor executor, int globalLimit, int tenantLimit,
                        int maximumRateKeys) {
        if (globalLimit < 1 || tenantLimit < 1 || maximumRateKeys < 1)
            throw new IllegalArgumentException("AMQP runtime limits must be positive");
    ...
    Admission acquire(String tenantId, AmqpProfile profile, int actionLimit,
    ...
    static final class Admission {
        ...
        Admission(AmqpRuntimeControls controls, GateLease tenant, GateLease profile, GateLease action) {
