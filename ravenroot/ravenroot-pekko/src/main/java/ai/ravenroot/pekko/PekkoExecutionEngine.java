...
/**
 * Apache Pekko Typed adapter. No Pekko type crosses the ExecutionEngine boundary.
 *
...
public final class PekkoExecutionEngine implements ExecutionEngine {
     ...
     * {@code ActorSystem}, for DeathWatch on a domain's guardian. Nothing outside this package's tests
     * has a reason to reach into the raw actor runtime -- see {@link ExecutionEngine}'s own contract
     * for why no Pekko type crosses the boundary anywhere else.
     */
    ...
    final class SubtreeDomain implements ExecutionDomain, DomainMembership {
         ...
         * guardian actor, for DeathWatch. {@code ActorRef<?>} rather than the private
         * {@code DomainCommand} protocol type -- a caller outside this class has no message it is
         * entitled to send this ref, only the right to watch whether it is still alive.
         */
    ...
    private Behavior<Command> idle(ActorContext<Command> context, StashBuffer<Command> stash,
        ...
        return Behaviors.receive(Command.class)
                .onMessage(Execute.class, command -> {
                    ...
                    // costs both guarantees at once: Pekko stops the actor, the caller's reply is
                    ...
                    // can no longer do its job. Rethrowing hands it back to Pekko's own fatal-error
                    ...
                    // pekko.jvm-exit-on-fatal-error away from the operator who enabled it.
                    try {
     ...
     * a terminated node. What it exists for is the path nobody plans: a failure Pekko supervised by
    ...
    private NodeContext nodeContext(NodeLifecycle lifecycle) {
        return new NodeContext() {
            ...
            @Override
            public Scheduler scheduler() {
                return PekkoExecutionEngine.this.scheduler();
            }
