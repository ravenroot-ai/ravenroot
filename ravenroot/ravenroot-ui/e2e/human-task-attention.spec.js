...
test('browser reload restores an exact task only after the service origin and token are reauthorized', async ({ page }) => {
  ...
  const relatedRequest = page.waitForRequest(request_ => {
    ...
    return url.pathname === '/v1/human-tasks/attention'
      && url.searchParams.get('graphVersion') === 'graph-v1'
      && url.searchParams.get('processInstanceId') === 'process-v1'
      && url.searchParams.get('nodeId') === 'human-confirmation';
  });
  await page.locator('[data-human-task-related-open]').click();
  const relatedUrl = new URL((await relatedRequest).url());
  expect(relatedUrl.searchParams.has('deploymentId')).toBe(false);
  await expect(page.locator('[data-human-task-related-status]')).toContainText('2 actionable related Human Tasks');
  await expect(page.locator('[data-human-task-related-list] [data-human-task-id]')).toHaveCount(1);
  await page.locator('[data-human-task-related-next]').click();
  await page.locator('[data-human-task-related-list] [data-human-task-id="task-2"]').click();
  expect(await page.evaluate(() => JSON.parse(
    localStorage.getItem('ravenroot.human-task.selection.v1')))).toEqual({
    serviceOrigin: SERVICE_ORIGIN, taskId: 'task-2', generation: 2,
  });
  ...
  expect(tasks[1].status).toBe('DENIED');
  expect(await page.evaluate(() => localStorage.getItem('ravenroot.human-task.selection.v1'))).toBeNull();
});

test('a document change retires a pending recovered related-task response', async ({ page }) => {
  await connectAndCreate(page);
  await runAndSelect(page);
  await page.locator('[data-human-task-id="task-1"]').click();
  ...
  let relatedStarted;
  let relatedFinished;
  const started = new Promise(resolve => { relatedStarted = resolve; });
  const released = new Promise(resolve => { releaseRelated = resolve; });
  const finished = new Promise(resolve => { relatedFinished = resolve; });
  const isRelatedUrl = raw => {
    const url = new URL(raw);
    return url.pathname === '/v1/human-tasks/attention'
      && url.searchParams.has('graphVersion') && url.searchParams.has('nodeId');
  };
  await page.route('**/v1/human-tasks/attention?*', async route => {
    if (!isRelatedUrl(route.request().url())) { await route.continue(); return; }
    try {
      relatedStarted();
      await released;
  ...
  expect(await page.evaluate(() => JSON.parse(
    localStorage.getItem('ravenroot.human-task.selection.v1')))).toEqual(locator);
});

test('reauthentication retires a pending recovered related-task response and its authority lease', async ({ page }) => {
  await connectAndCreate(page);
  await runAndSelect(page);
  await page.locator('[data-human-task-id="task-1"]').click();
  const locator = await page.evaluate(() => JSON.parse(
  ...
  await page.route('**/v1/human-tasks/attention?*', async route => {
    ...
    if (!url.searchParams.has('graphVersion') || !url.searchParams.has('nodeId')) {
  ...
  await page.locator('[data-human-task-related-open]').click();
  await started;
  // Native modal semantics correctly block an outside pointer click. DOM activation exercises the
  // unchanged revoke handler while this lifecycle test deliberately keeps the modal open.
...
test('a committed decision response is UI-inert after its recovered document is replaced', async ({ page }) => {
  await connectAndCreate(page);
  await runAndSelect(page);
  await page.locator('[data-human-task-id="task-1"]').click();
  const locator = await page.evaluate(() => JSON.parse(
  ...
  let releaseReply;
  let decisionCommitted;
  let routeFinished;
  const committed = new Promise(resolve => { decisionCommitted = resolve; });
  const released = new Promise(resolve => { releaseReply = resolve; });
  const finished = new Promise(resolve => { routeFinished = resolve; });
  await page.route('**/v1/human-tasks/*/confirmation/resolve?*', async route => {
    try {
  ...
  await page.evaluate(() => window.ravenroot.openDocument({ name: 'replacement.graphml' }));
  await expect(page.locator('#human-task-dialog')).toBeHidden();
  releaseReply();
  await finished;
  expect(await page.evaluate(() => JSON.parse(
